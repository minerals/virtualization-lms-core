...
trait ScalaCodegen extends GenericCodegen with Config {
  ...
  override def remap[A](m: Manifest[A]): String = {
    ...
    else { // default remapping is to keep the same type name as the Manifest
      val targs = m.typeArguments
      if (targs.length > 0) { // if type has type arguments, remap them
        val ms = m.toString
        if (ms.indexOf("[") < 1) super.remap(m) // don't understand type
        ms.take(ms.indexOf("[")+1) + targs.map(remap(_)).mkString(", ") + "]"
      }
      else m.toString
    }
  }
